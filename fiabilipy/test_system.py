...
class TestSystem(unittest2.TestCase):
    ...
    def test_successpaths(self):
        ...
        """
        paths = {
            'simple': {('E', self.alim[0], self.motors[0], 'S')},
            'series-parallel': {('E', self.alim[0], self.motors[0], 'S'), ('E', self.alim[1], self.motors[1], 'S')},
            'parallel-series': {('E', self.alim[0], self.motors[0], 'S'), ('E', self.alim[0], self.motors[1], 'S'),
                                ('E', self.alim[1], self.motors[1], 'S'), ('E', self.alim[1], self.motors[0], 'S')},
            'complex': {('E', self.alim[0], self.motors[0], 'S'), ('E', self.alim[1], self.motors[0], 'S'),
                        ('E', self.alim[1], self.motors[1], 'S'), ('E', self.alim[2], self.motors[1], 'S')},
            'voter': {('E', self.voter, self.motors[0], 'S'), ('E', self.voter, self.motors[1], 'S')},
        }
    ...
    def test_minimalcuts(self):
        ...
        cuts = {
            1: {
                'simple': {frozenset([self.alim[0]]), frozenset([self.motors[0]])},
                'series-parallel': set(),
                'parallel-series': set(),
                'complex': set(),
                'voter': {frozenset([self.voter])},
            },
            2: {
                'simple': {frozenset([self.alim[0]]), frozenset([self.motors[0]])},
                'series-parallel': {frozenset([self.alim[1], self.alim[0]]), frozenset([self.alim[1], self.motors[0]]),
                                    frozenset([self.motors[1], self.alim[0]]),
                                    frozenset([self.motors[1], self.motors[0]])},
                'parallel-series': {frozenset([self.alim[1], self.alim[0]]),
                                    frozenset([self.motors[1], self.motors[0]])},
                'complex': {frozenset([self.motors[0], self.motors[1]])},
                'voter': {frozenset([self.voter]), frozenset([self.motors[0], self.motors[1]])},
            },
        }
